...
namespace gui {
	...
	void Gui::setInput(InputInterface* input)
	{
		mInput = input;
	}

	InputInterface* Gui::getInput() const
	{
		return mInput;
	}
	...
	void Gui::logic()
	{
		if (mTop == NULL)
		{
			throw GUI_EXCEPTION("No top widget set");
		}
	...
	void Gui::draw()
	{
		...
		if (mGraphics == NULL)
		{
			throw GUI_EXCEPTION("No graphics set");
		}
	...
	void Gui::handleMouseInput()
	{
		while (!mInput->isMouseQueueEmpty())
		{
			...
			case MouseInput::WHEEL_MOVED_UP:
				...
				break;
			default:
				throw GUI_EXCEPTION("Unknown mouse input type.");
				break;
			}
		}
	}
			...
			if (widget->isEnabled() || force)
			{
					...
					++it)
				{
					...
					case MouseEvent::CLICKED:
						...
						break;
					default:
						throw GUI_EXCEPTION("Unknown mouse event type.");
					}
				}
			...
			if (widget->isEnabled())
			{
					...
					++it)
				{
					...
					case KeyEvent::RELEASED:
						...
						break;
					default:
						throw GUI_EXCEPTION("Unknown key event type.");
					}
				}
			}
